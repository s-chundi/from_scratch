...
class SmollmDataset(Dataset):
    def __init__(
        ...
        if split == "train":
            ds = load_dataset("HuggingFaceTB/smollm-corpus", "cosmopedia-v2", split="train[:5000]")
        elif split == "test":
            ds = load_dataset("HuggingFaceTB/smollm-corpus", "cosmopedia-v2", split="train[5000:5150]")
        else:
